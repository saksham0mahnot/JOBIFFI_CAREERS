...
import { FiPlus, FiBriefcase, FiFileText, FiLogOut, FiCheck, FiX, FiEdit2, FiTrash2, FiExternalLink } from 'react-icons/fi';
...
const EmployeeDashboard: React.FC = () => {
    ...
    const handleEditJob = (job: any) => {
        ...
        setJobForm({
            title: job.title || '',
            company: job.company || 'Jobiffi',
            location: job.location || '',
            description: job.description || '',
            salary: job.salary || '',
            jobType: job.jobType || 'Full-time',
            refCode: job.refCode || '',
            domain: job.domain || 'Software Engineering'
        });
        setActiveTab('post');
    };
                ...
                <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden min-h-[600px]">
                        ...
                        <div className="p-6">
                            {myJobs.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {myJobs.map(job => (
                                        <div key={job._id} className="p-5 border border-gray-100 rounded-2xl hover:border-blue-200 hover:shadow-md transition-all flex flex-col md:flex-row justify-between items-start md:items-center group bg-white">
                                            <div className="mb-4 md:mb-0">
                                                <h3 className="text-xl font-bold text-gray-900 mb-1">{job.title}</h3>
                                                <div className="flex flex-wrap gap-2 text-sm text-gray-500 font-medium">
                                                    <span className="bg-gray-100 px-2.5 py-1 rounded-md">{job.location}</span>
                                                    <span className="bg-blue-50 text-blue-700 px-2.5 py-1 rounded-md">{job.jobType}</span>
                                                    <span className="bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-md">{job.domain}</span>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0">
                                                ...
                                                <button
                                                    ...
                                                    className="flex-1 md:flex-none bg-[#0122c5] text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-md hover:bg-blue-800 hover:shadow-lg transition-all"
                                                >
                                                    View Applications
                                                </button>
                                            </div>
                    ...
                    {activeTab === 'post' && (
                        <form onSubmit={handlePostJob} className="p-8 max-w-2xl">
                            ...
                            >
                                {loading ? 'Saving...' : (editingJobId ? 'Update Job Posting' : 'Launch Job Posting')}
                            </button>
                        </form>
                    )}
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left">
                                        <thead className="bg-gray-50">
                                            <tr className="border-b border-gray-200 text-gray-500 text-sm font-semibold text-left">
                                                <th className="px-6 py-4 rounded-tl-xl">Applicant</th>
                                            ...
                                            {applications.map(app => (
                                                <tr key={app._id} className="hover:bg-blue-50/30 transition-colors">
                                                    <td className="px-6 py-5">
                                                        <div className="font-bold text-gray-900 text-sm">{app.applicant?.name || 'Unknown Candidate'}</div>
                                                        <div className="text-xs text-gray-500 mt-0.5">{app.applicant?.email}</div>
                                                    </td>
                                                    <td className="px-6 py-5">
                                                        {app.resume ? (
                                                            <a
                                                                ...
                                                                className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#0122c5] hover:text-blue-800 hover:underline bg-blue-50 px-3 py-1.5 rounded-lg transition-colors border border-blue-100"
                                                            >
                                                                <FiExternalLink /> View Resume
                                                            </a>
                                                        ) : (
                                                            <span className="text-gray-400 text-sm font-medium italic">Not provided</span>
                                                        )}
                                                    </td>
                                                    <td className="px-6 py-5">
                                                    ...
                                                    <td className="px-6 py-5 text-right flex gap-3 justify-end items-center h-full pt-6">
                                                        <button
                                                            ...
                                                            className={`p-2 rounded-lg transition-colors shadow-sm border ${app.status === 'accepted' ? 'bg-green-600 text-white border-green-600' : 'bg-green-50 text-green-600 border-green-200 hover:bg-green-100'}`}
                                                            title="Accept / Move Forward"
                                                        >
                                                            <FiCheck size={18} />
                                                        </button>
                                                        <button
                                                            onClick={() => handleUpdateStatus(app._id, 'rejected')}
                                                            className={`p-2 rounded-lg transition-colors shadow-sm border ${app.status === 'rejected' ? 'bg-red-600 text-white border-red-600' : 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100'}`}
